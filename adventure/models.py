...
class Adventure(models.Model): #Primer menú
    ...
    menus = models.ForeignKey('adventure.Chapter', through='adventure.AdventureConf', related_name='adventure', on_delete=models.CASCADE) #Selección de capítulo (I, II, III, IV o V)
...
class Chapter(models.Model): #Segundo menú
    ...
    unlocked_info = models.CharField(max_length=50) #Opens week 2 | Opens week 5
    menus = models.ManyToManyField('adventure.Champion', through='adventure.AdventureConf', on_delete=models.CASCADE) #relación Chapter/Champion
...
class Champion(models.Model): #Tercer menú
    ...
    unlocked_info = models.CharField(max_length=100) #Opens week 2 | Opens week 5
    hero = models.ManyToManyField('adventure.Hero', related_name='champions', on_delete=models.CASCADE) #Selección del hero.
    menus = models.ManyToManyField('adventure.Power', through='adventure.AdventureConf', on_delete=models.CASCADE) #relación Champion/Power
...
class AdventureConf(models.Model): #Tabla de relación
    ...
    champ = models.ForeignKey('adventure.Champion', on_delete=models.CASCADE) #Indica el hero
    power = models.ForeignKey('adventure.Power', on_delete=models.CASCADE) #Indica el hero power
    deck = models.ForeignKey('adventure.Deck', on_delete=models.CASCADE) #Indica el Deck
    enemy = models.ForeignKey('adventure.Challenge', on_delete=models.CASCADE) #Indica el Enemy
    info = models.PositiveIntegerField(default=0) #Dalaran. 0/5 Chapters Completed
    ...
    new = models.BooleanField() #Indica si se trata de un Item nuevo con "New"