...
class DeckCard(models.Model):
    ...
    quantity = models.IntegerField(validators=[MaxValueValidator(2), MinValueValidator(1)])
    ...
    def save(self, *args, **kwargs):
        total = DeckCard.objects.filter(card_id=self.card.id, deck_id=self.deck.id).aggregate(total=Sum("quantity"))["total"]
        if total is None:
            total = 0
        total_copies = total + self.quantity 
        if self.card.quality != QualityCard.LEGENDARY and total_copies > 2 or self.card.quality == QualityCard.LEGENDARY and total_copies > 1:
